...
var AIMLUnicode = function (botAttributesParam) {
    var self = this;
    botAttributes = botAttributesParam;

    this.loadAIMlFile = function (fileArray) {
        ...
        for (var i = 0; i < innerNodes.length; i++) {
            ...
            if (innerNodes[i].name === 'bot') {
                    ...
                    for (var c in innerNodes[i].children) {
                        ...
                        if (child.name === 'li') {
                            if (child.attributes.value == undefined
                                || getUpperCaseExceptError(storedVariableValues[child.attributes.name]) === child.attributes.value.toUpperCase()) {
                ...
                // condition tag specification: multi condition tag
                    ...
                    if (getUpperCaseExceptError(storedVariableValues[innerNodes[i].attributes.name]) === innerNodes[i].attributes.value.toUpperCase()) {
                        text = text + resolveSpecialNodes(innerNodes[i].children);
                    }
                }
                // condition tag specification: single name list condition tags
                    ...
                    for (var c in innerNodes[i].children) {
                        ...
                        if (child.name === 'li') {
                            if (child.attributes.value === undefined
                                || getUpperCaseExceptError(storedVariableValues[innerNodes[i].attributes.name]) === child.attributes.value.toUpperCase()) {
